...
export default function Navigation() {
  ...
  return (
    <>
      <nav className="bg-brand-lightBlue border-b border-gray-300">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16">
            <div className="flex items-center">
              ...
              >
                <Image
                  src="/images/brancoy_logo_black.webp"
                  alt="Brancoy HI Engine Logo"
                  width={128}
                  height={36}
                ...
                <Link
                  ...
                  className={`px-3 py-2 rounded-md text-sm font-medium transition-colors ${
                    pathname?.endsWith(href)
                      ...
                      : 'text-slate-700 hover:text-brand-blue hover:bg-sky-200'
                  }`}
                >
                  {label}
                </Link>
            ...
            <div className="flex items-center space-x-5">
                      ...
                      {(!loading && isAdmin) && (
                        <Link
                          ...
                          className={`text-sm font-medium transition-colors ${
                            isAdminPath
                              ? 'text-white bg-brand-blue px-3 py-2 rounded-md'
              ...
              <button
                onClick={toggleMenu}
                className="sm:hidden inline-flex items-center justify-center p-2 rounded-md text-slate-700 hover:text-brand-blue hover:bg-sky-200 transition-colors"
                aria-expanded="false"
              >
      ...
      <div
        ...
        } fixed inset-y-0 right-0 w-full sm:hidden bg-brand-lightBlue transform transition-transform duration-300 ease-in-out z-50`}
      >
        <div className="pt-20 pb-3 space-y-1 px-4">
          ...
          {(!loading && isAdmin && isAdminPath) ? (
            <>
              ...
              <Link
                href="/"
                className="block px-3 py-2 mt-4 rounded-md text-base font-medium text-slate-700 hover:text-brand-blue hover:bg-sky-200 border-t border-gray-300"
              >
                {t('backToSite')}
              </Link>
            </>
              ...
              <>
                {session ? (
                  <>
                    <Link
                      href="/account/settings"
                      className="block px-3 py-2 rounded-md text-base font-medium text-slate-700 hover:text-brand-blue hover:bg-sky-200"
                    >
                      {t('account.title')}
                    </Link>
                    {(!loading && isAdmin && !isAdminPath) && (
