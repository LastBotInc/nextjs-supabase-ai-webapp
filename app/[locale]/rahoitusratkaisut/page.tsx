...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations({ locale, namespace: "FinancialSolutions.meta" });

  return generateLocalizedMetadata({
    title: t("title"),
    description: t("description"),
    locale,
    namespace: "FinancialSolutions",
    path: "/rahoitusratkaisut",
  });
}
...
export default async function Page({ params }: Props) {
  ...
  const t = await getTranslations({ locale, namespace: "FinancialSolutions" });

  // Get raw data objects
  const hero = t.raw("hero");
  const leasingTypes = t.raw("leasingTypes");
  const benefits = t.raw("benefits");
  const minileasing = t.raw("minileasing");
  const equipmentFinancing = t.raw("equipmentFinancing");
  const faq = t.raw("faq");
  const cta = t.raw("cta");
  ...
  return (
    <PageWrapper>
      ...
      <BasicLayout contentPalette="light-gray">
        <Heading2>{leasingTypes.heading}</Heading2>
        <Paragraph>{leasingTypes.subheading}</Paragraph>

        {leasingTypes.texts?.map((text: string, idx: number) => (
          <Paragraph key={idx}>{text}</Paragraph>
        ))}

        <Columns>
          {leasingTypes.cases?.map(
      ...
      {/* MINILEASING SECTION */}
      <TwoColumnLayout contentPalette="maantie">
        <FlexLayout.Column>
          <Heading2>{minileasing.heading}</Heading2>
          <Paragraph>{minileasing.subheading}</Paragraph>

          {minileasing.texts?.map((text: string, idx: number) => (
            <Paragraph key={idx}>{text}</Paragraph>
          ))}
        </FlexLayout.Column>
        <FlexLayout.Column>
          <List>
            {minileasing.list?.map((item: string, idx: number) => (
      ...
      <TwoColumnLayout contentPalette="beige">
        <FlexLayout.Column>
          <Heading2>{equipmentFinancing.heading}</Heading2>
          <Paragraph>{equipmentFinancing.subheading}</Paragraph>

          {equipmentFinancing.texts?.map((text: string, idx: number) => (
            <Paragraph key={idx}>{text}</Paragraph>
          ))}
        </FlexLayout.Column>
        <FlexLayout.Column>
          <List>
            {equipmentFinancing.list?.map((item: { term: string; description: string }, idx: number) => (
              <List.Item key={idx}>
