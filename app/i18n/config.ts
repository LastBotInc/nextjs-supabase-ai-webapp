...
export async function getEnabledLocales(): Promise<Locale[]> {
  ...
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_SITE_URL}/api/languages`,
    );
    ...
    if (!enabledLocales?.length) {
      console.log(
        "No enabled locales found, using static locales:",
        staticLocales,
      );
      return staticLocales;
    }
...
async function loadNamespacesUsingImport(
  locale: Locale,
  ...
  try {
    ...
    try {
      ...
      console.warn(`Navigation namespace not found for ${locale}`, error);
    }
    ...
    for (const namespace of availableNamespaces) {
      try {
        ...
        namespaces[namespace] = namespaceModule;
      } catch (error) {
        // Log specific import errors
        console.error(
          `Failed to import namespace '${namespace}' for locale '${locale}':`,
          error,
        );
    ...
    console.error(
      `Error loading namespaces using import for ${locale}:`,
      error,
    );
    throw error;
  }
...
async function loadMonolithicTranslation(
  locale: Locale,
): Promise<NestedMessages> {
  try {
    ...
    try {
      ...
      console.error(
        "Critical error: Failed to load even the default locale translation",
      );
      return {};
    } catch (error) {
...
export async function getI18nConfig(
  ...
  try {
    ...
    console.log(
      `Successfully loaded namespace-based translations for ${locale} using imports`,
    );

    // If no translations were loaded, fall back to monolithic translation
    if (Object.keys(messages).length === 0) {
      console.warn(
        `No translations loaded for ${locale}, trying monolithic fallback`,
      );
    ...
    try {
      ...
      console.error(
        `Failed to load even monolithic fallback for ${locale}:`,
        fallbackError,
      );
      ...
      if (locale !== defaultLocale) {
        console.warn(
          `Attempting to use default locale (${defaultLocale}) as last resort`,
        );
        try {
