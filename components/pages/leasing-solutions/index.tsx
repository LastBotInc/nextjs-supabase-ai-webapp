...
import { IconCar, IconTruck, IconTools, IconChart, IconCalendar } from '@/app/components/Icons'
...
const blurDataURL = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mN88P//fwAJMwPbA7ugEgAAAABJRU5ErkJggg=='
...
export default function LeasingSolutionsPage({ locale }: LeasingSolutionsPageProps) {
  ...
  const calculatePayment = () => {
    ...
    const payment = (
      (loanAmount * monthlyInterestRate * Math.pow(1 + monthlyInterestRate, term)) -
        ...
        <div className="flex flex-col lg:flex-row items-center gap-12">
          ...
          <div className="lg:w-1/2 bg-gray-50 p-6 rounded-lg shadow">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('financialLeasing.calculator.vehicle')}</label>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('financialLeasing.calculator.downpayment')}</label>
                <div className="relative rounded-md shadow-sm">
                  <span className="absolute inset-y-0 left-0 pl-3 flex items-center text-gray-500">€</span>
                  <input
                    type="number"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('financialLeasing.calculator.term')}</label>
                <input
                  type="number"
                  name="term"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('financialLeasing.calculator.residual')}</label>
                <div className="relative rounded-md shadow-sm">
                  <span className="absolute inset-y-0 left-0 pl-3 flex items-center text-gray-500">€</span>
                  <input
                    type="number"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('financialLeasing.calculator.interest')}</label>
                <div className="relative rounded-md shadow-sm">
                  <input
                    type="number"
                    name="interestRate"
              ...
              {monthlyPayment !== null && (
                <div className="mt-4 p-4 bg-gray-100 rounded-md">
                  <p className="font-medium text-gray-900">{t('financialLeasing.calculator.result')} <span className="text-xl font-bold text-blue-600">€{monthlyPayment}</span></p>
                    ...
                    <tr key={row.key}>
                      ...
                      {row.included.map((status, index) => (
                        <td key={index} className="px-6 py-4 whitespace-nowrap text-sm text-center text-gray-700">
                          {status === true && <span className="text-green-600 font-medium">{t('maintenanceLeasing.comparison.included')}</span>}
                          {status === false && <span className="text-red-600 font-medium">{t('maintenanceLeasing.comparison.excluded')}</span>}
                          {status === 'optional' && <span className="text-yellow-600 font-medium">{t('maintenanceLeasing.comparison.optional')}</span>}
                        </td>
                      ))}
                    </tr>
                  ))}
            ...
            {[1, 2, 3, 4, 5, 6, 7].map((i) => (
              ...
              >
                ...
                >
                  <span>{t(`faq.q${i}`)}</span>
                  <svg
                    className={`w-5 h-5 transition-transform text-blue-600 ${openFaqIndex === i ? 'transform rotate-180' : ''}`}
                    fill="none" 
                    viewBox="0 0 24 24" 
                    stroke="currentColor"
                  >
